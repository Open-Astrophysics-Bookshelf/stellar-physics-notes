...
\section{Examples}\label{s.LE-examples}
...
\frac{aT^{4}}{3P} = \beta = \mathrm{const.},
\end{equation}
that is, the radiation pressure is a fixed fraction of the total pressure everywhere.  Solving for $T$ in terms of $P$ and $\beta$,
\[ T = \left(\frac{3\beta P}{a}\right)^{1/4}, \]
and inserting this into equation~(\ref{e.eos-with-rad}) gives us a simple EOS,
\begin{equation}\label{e.beta-eos}
P = \left[\left(\frac{\kB}{\mu\mb}\right)^{4}\frac{3}{a}\right]^{1/3}\left[\frac{\beta}{(1-\beta)^{4}}\right]^{1/3} \rho^{4/3}.
\end{equation}
