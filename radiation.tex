...
and for blackbody radiation, if we only integrate over $0\le \mu\le 1$,
\[ F_{\nu} = \pi B_{\nu}.\]
If we integrate $F_{\nu}$ over all frequencies, we recover the result,
...
\mu\frac{\dif I_{\nu}}{\dif \tau_{\nu}} = -I_{\nu}.
\end{equation}
Here we have set $\unitk\cdot\grad = \mu(\dif/\dif s)$, where $\dif s$ is a infinetesimal along the path of the ray, and further have defined the \emph{optical depth} as
...
\[ I_{\nu}(\tau_{\nu}) = I_{\nu}(t)\exp\left(-\frac{\tau_{\nu}-t}{\mu}\right). \]
where $t$ is some reference optical depth. Note that $\rho\kappa=\ell^{-1}$, so equation~(\ref{e.optical-depth-def}) is just $\tau = \int \dif s/\ell$, i.e., it expresses distance by counting the number of mean free pathlengths traversed.
