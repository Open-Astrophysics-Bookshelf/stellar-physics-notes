...
Let's consider a plasma having many species, each with charge $Z_{i}$.  To simplify the algebra, we'll take electrons to have $Z = -1$. About any selected charge $j$, the particles of type $i$ will arrange themselves according to Boltzmann's law,
...
\frac{1}{r}\frac{\partial^{2}}{\partial r^{2}}\left(r\Phi\right) = \left[\frac{4\pi e^{2}}{\kB T}\sum_{i} n_{i0}Z_{i}^{2}\right]\Phi \equiv \lambdaD^{-2}\Phi.
\end{equation}
...
With a little bit of algebra, one can write the Coulomb energy per unit volume of the plasma as
\begin{equation}\label{e.total-energy-plasma-dh}
\frac{E_{\mathrm{Coul}}}{V} = -\frac{\sqrt{3}}{2} \Gamma^{3/2}\; n\kB T
\end{equation}
with the definition of $\Gamma$ for a multi-component plasma being
...
\begin{equation}\label{e.pressure-correction}
 P_{\mathrm{Coul}} \approx -\frac{e^{3}}{3}\left(\frac{\pi}{\kB T}\right)^{1/2}\left(\langle Z^{2}\rangle n\right)^{3/2}
  = -\frac{1}{3}\frac{\sqrt{3}}{2}\Gamma^{3/2}\;n\kB T .
\end{equation}
