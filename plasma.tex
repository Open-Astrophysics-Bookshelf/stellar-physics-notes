...
Imagine a typical charged particle in this plasma.  Very close to the particle, we expect the electrostatic potential to be that of an isolated charge $\Phi = q/r$. Far from the particle, there will be many other particles surrounding it, and we may expect that the potential to be screened. For example, a positive ion will tend to attract electrons to be somewhat closer, on average to it---the ion \emph{polarizes} the plasma.  As a result of this polarization, the potential from this particular ion should go to zero much faster than $1/r$ due to the ``screening'' from the enhanced density of negative charges.

To sharpen this discussion, let's consider a plasma having many species, each with charge $Z_{i}$.  (To simplify the algebra, let's take electrons to have $Z = -1$). About any selected charge $j$, the particles of type $i$ will arrange themselves according to Boltzmann's law,
\begin{equation}\label{e.ion-boltzmann}
n_{i}(r) = n_{i0}\exp\left[-\frac{Z_{i}e\Phi(r)}{\kB T}\right].
\end{equation}
Here $n_{i0}$ is the density of particle $i$ far from the charge $j$, and $r$ is the distance between particles $i$ and $j$.  To solve for the potential, we can use Poisson's equation,
