...
\newthought{Now we are ready to decompose our perturbed system into normal modes,} with the perturbed quantities varying in time as $\exp(i\omega t)$.
...
\end{eqnarray*}
Here we used $\vxi\vdot\grad \ln P = \xi_{r}(\dif/\dif r) \ln P = -\xi_{r}\rho g/P = -\xi_{r}/H$, where $H$ is the pressure scale height.  It makes physical sense that this scale will enter: modes with radial wavelengths much less than $H$ shouldn't be affected by the background stratification.
...
In the limit where the radial wavelength is much less than a pressure scale height, $|K|H\gg 1$, equation~(\ref{e.full-dispersion-equation}) simplifies to
