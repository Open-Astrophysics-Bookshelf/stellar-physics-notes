...
For small $A$, we see that the most bound nuclei have $eta = 0$, i.e., equal numbers of neutrons and protons.  As $A$ increases, however, the Coulomb term becomes important.  Expanding the last two terms and combining,
\[ \frac{a_{C}}{4} A^{2/3} - a_{C}A^{2/3}\eta + \left[ a_{C}A^{2/3} + a_{A} \right] \eta^{2} ,\]
we see that for large $A$, $\eta < 0$, i.e., $N>Z$ and the most bound nuclei are neutron-rich. The nuclei for which $-B/A$ is a minimum define the \emph{valley of stability}.
