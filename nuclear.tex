...
\end{equation}
where $M$ is the total mass of a nucleus with $N$ neutrons and $Z$ protons. From the form of this definition $B > 0$ for bound nuclei. The fact that the nuclear interaction binds the nucleons into a ``nuclear matter'' means that one can make a crude model of the nucleus as a liquid drop, and fit this formula to experimental masses.  This model, due to Weiz\"acker, is
\begin{equation}\label{e.weizacker-fmla}
-B(N, Z) = a_{V} A + a_{S}A^{2/3} + a_{A}\frac{(N-Z)^{2}}{A} + a_{C}\frac{Z^{2}}{A^{1/3}}.
\end{equation}
Here $A = N+Z$, $m_{p} = 938.272\nsp\MeV/c^{2}$, and $m_{n} = 939.565\nsp\MeV/c^{2}$. Using the online fitting routine from the \href{http://128.95.95.61/~intuser/ld.html}{Institute for Nuclear Theory}, I obtained the coefficients listed in Table~\ref{t.liquid-drop}.
...
\end{equation}
Here $\alpha = e^{2}/(\hbar c) = 1/137$ is the fine-structure constant and $\hbar c = 197 \nsp\MeV\usp\fermi$.  In ``nuclear units,'' $e^{2} = \alpha \hbar c = (197\nsp\MeV\usp\fermi)/137 = 1.44\nsp\MeV\usp\fermi = 1440\nsp\keV\usp\fermi$. Remember these numbers!  Clearly the cross-section for a reaction between our pair of particles is controlled by the probability of tunneling through the Coulomb potential.
