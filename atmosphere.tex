...
\section{Requirement for convection in the atmosphere}\label{s.atmosphere-convection}
...
\section{An irradiated atmosphere}\label{s.two-stream}
...
\begin{equation}\label{e.cross-section}
	 \sigma_{\nu} = \left(\frac{\pi e^{2}}{m_{e}c}\right) f \phi_{nu}, 
\end{equation}
where $\phi_{\nu}$ is the line profile (dimension $\sim \Hz^{-1}$) and $f$ is a dimensionless cross-section called the \emph{oscillator strength}.
