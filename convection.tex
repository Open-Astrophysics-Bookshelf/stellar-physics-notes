...
As the blob rises, it displaces some of the surrounding fluid. Archimedes tells us that if the displaced fluid is less massive than the blob, then the blob will sink.  We can rephrase this in terms of the volume occupied by a unit mass of fluid $V$: if
\begin{eqnarray}
\lefteqn{V[P(z+h),S(z+h)] - V_{b}[P_{b}(z+h),S_{b}(z+h)] =}\nonumber\\
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\textwidth]{convective.png}}
...
Expanding the left-hand side of equation~(\ref{e.archimedes}) gives
\[
...
\emph{A mixture of uniform composition is unstable to convection if the local temperature gradient is steeper than an adiabat,.}
...
Let's recast equation~(\ref{e.navier-stokes-with-visc}) into dimensionless form by scaling our variable: let $L$ and $U$ represent the characteristic length and velocity scales, and define the dimensionless variables $\bar{x} = x / L$ and $\bar{u} = u / U$. This choice then implicitly defines the time variable, $\bar{t} = t\cdot U/L$. Upon changing to the variables $\bar{x}$, $\bar{u}$, and $\bar{t}$, and writing the equation of state as $P = c_{s}^{2} \rho$ (appropriate for adiabatic flow---we are ignoring heat conduction), we obtain the equation
