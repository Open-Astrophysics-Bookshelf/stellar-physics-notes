...
For this exercise, we'll use the setup from assignment 1, namely a contracting, pre-main-sequence star of mass $\val{1}{\Msun}$.  Download the folder |convection/1M-convection| and place it into your projects folder. 
...
We argued in \S~3.3 of the notes that convection was very efficient in the solar interior.  By efficient, we mean that a very slow convective velocity $v_{\mathrm{conv}}$ is sufficient to carry the flux.  As a result, the difference between the temperature gradient and the adiabatic one is also expected to be very small: $\nabla - \nablaad \ll 1$, where $\nabla \equiv \dif\ln T/\dif\ln P$ and $\nablaad=(\partial\ln T/\partial\ln P)_{s}$. We also worked through a linear stability analysis in \S~3.2 and found (eq.~3.20) the \emph{Brunt-V\"ais\"al\"a} frequency $N^{2}$. This is the oscillation frequency for a fluid element that is adiabatically displaced in the radial direction; $N^{2}< 0$ in a region with $\nabla > \nablaad$ and therefore unstable to convection.
