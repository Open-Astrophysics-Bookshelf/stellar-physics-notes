...
In this experiment, we are doing four runs, so we've taken the `|rn|' script from `|$MESA_DIR/star/work/|' and renamed it `|rn1|'. We then made a bespoke `|rn|' script that loops over masses,
\VerbatimInput[firstline=22,lastline=25]{radiation/beta-eddington/rn}
with the function `|do_one|' copying the file `|inlist_mM|' to `|inlist|' and then calling `|rn1|'. The inlists are all the same and load settings in `|inlist_common|' and `|inlist_pgstar|', with one exception in `|&controls|', for example in `|inlist_3M|':
...
Settings that are common to all runs are placed in `|inlist_common|'. Three of note are
...
With the computation for each star established, we can now focus on what the gas and radiation pressure are doing in the interior.
