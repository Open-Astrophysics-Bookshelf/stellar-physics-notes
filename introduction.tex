...
\item Equation~(\ref{e.hydrostatic}) must in general be solved numerically for a real equation of state $P = P(\rho)$, but it is useful to construct a toy model to gain insight.  Suppose the sun had a constant density throughout, and suppose that the gas pressure is that of an ideal gas.  Compute the pressure and temperature as functions of the radial coordinate $r$ for a sun made of pure hydrogen. Express their central values in terms of the total mass $M$, radius $R$, and mean molecular weight $\mu$.  To mimic the effect of chemical evolution, for a fixed radius, how does the central temperature change as the composition goes from a ``solar mixture'' (hydrogen mass fraction 0.7) to a pure He composition?  If the nuclear reaction rate depends on temperature, what would this do the luminosity, in the absence of any other changes?
